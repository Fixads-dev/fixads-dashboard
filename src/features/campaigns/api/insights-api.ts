...
/**
 * Validate that an asset group ID contains only digits.
 * Prevents path traversal when interpolating into URL paths.
 */
export function validateAssetGroupId(assetGroupId: string): void {
  if (!/^\d+$/.test(assetGroupId)) {
    throw new Error("Invalid asset group ID");
  }
}
...
export const insightsApi = {
  ...
  getPlacements: (accountId: string, campaignId: string, limit: number = 100) => {
    ...
    return apiMethods.get<PlacementsResponse>(
  ...
  getTopCombinations: (accountId: string, assetGroupId: string) => {
    validateAssetGroupId(assetGroupId);
    return apiMethods.get<TopCombinationsResponse>(
  ...
  getSegmentedPerformance: (
    ...
    validateCampaignId(campaignId);
    return apiMethods.get<SegmentedPerformanceResponse>(
  ...
  getSignals: (accountId: string, assetGroupId: string) => {
    validateAssetGroupId(assetGroupId);
    return apiMethods.get<SignalsResponse>(
  ...
  addSearchThemeSignal: (accountId: string, assetGroupId: string, searchTheme: string) => {
    validateAssetGroupId(assetGroupId);
    return apiMethods.post<{ signal_id: string; success: boolean }>(
  ...
  getProducts: (accountId: string, campaignId: string, limit: number = 100) => {
    validateCampaignId(campaignId);
    return apiMethods.get<ProductsResponse>(
  ...
  getProductGroups: (accountId: string, campaignId: string) => {
    validateCampaignId(campaignId);
    return apiMethods.get<ProductGroupsResponse>(
  ...
  getSimulations: (accountId: string, campaignId: string) => {
    validateCampaignId(campaignId);
    return apiMethods.get<SimulationsResponse>(
  ...
  getDemographics: (
    ...
    validateCampaignId(campaignId);
    return apiMethods.get<DemographicsResponse>(
  ...
  getTimePerformance: (
    ...
    validateCampaignId(campaignId);
    return apiMethods.get<TimePerformanceResponse>(
  ...
  getAuctionInsights: (
    ...
    validateCampaignId(campaignId);
    return apiMethods.get<AuctionInsightsResponse>(
